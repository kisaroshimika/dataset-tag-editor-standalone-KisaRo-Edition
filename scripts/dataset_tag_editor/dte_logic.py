...
class DatasetTagEditor(Singleton):
    ...
    def interrogate_single_image(self, image: Image.Image, interrogator_name: str, threshold: float, sort_by_confidence: bool = False):
        """単一画像のタグ抽出（確率付き）
        
        Args:
            sort_by_confidence: Trueの場合は確率降順ソート、Falseの場合はCSV定義順（一括処理と同じ並び）
        
        Returns:
            list[tuple[str, float]]: (タグ名, 確率) のリスト
        """
        from tagger import get_replaced_tag
        try:
            img = get_square_rgb(image)
        except:
            return []
        
        for it in self.INTERROGATORS:
            if it.name() == interrogator_name:
                # tagger_inst を持つもの（WD系 / DeepDanbooru / Z3D）
                if hasattr(it, 'tagger_inst'):
                    with it as tg:
                        labels = tg.tagger_inst.apply(img)
                        # ratingタグの除外
                        if isinstance(tg, (taggers_builtin.WaifuDiffusion, taggers_builtin.WaifuDiffusionTimm)):
                            if not tg.use_rating:
                                labels = labels[4:]
                        elif isinstance(tg, taggers_builtin.DeepDanbooru):
                            if not tg.use_rating:
                                labels = labels[:-3]
                        # 閾値フィルタ（CSV定義順を維持）
                        filtered = [
                            (get_replaced_tag(tag), float(prob))
                            for tag, prob in labels if prob > threshold
                        ]
                        # 確率降順ソート（オプション）
                    ...
                    # BLIP / GIT 等のキャプションモデル（確率なし）
                    with it as cap:
                        tags = cap.predict(img)
