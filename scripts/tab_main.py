...
def update_gallery():
    ...
    ui.gallery_state.register_value(
        i18n("displayed_images"), f"{displayed_image_num} / {total_image_num} {i18n('total')}"
    )
    ui.gallery_state.register_value(
        i18n("current_tag_filter"),
    ...
    ui.gallery_state.register_value(
        i18n("current_selection_filter"),
        f"{len(ui.filter_by_selection.path_filter.paths)} {i18n('images')}",
    )
    return [
        imgs,
        -1,
...
def on_ui_tabs():
    ...
    with gr.Blocks(analytics_enabled=False) as interface:
        gr.HTML(
            value=i18n("note_extension_works")
        )

        ui.toprow.create_ui(cfg_general)

        with gr.Accordion(label=i18n("reload_save_config"), open=False):
            with gr.Row():
                btn_reload_config_file = gr.Button(value=i18n("reload_settings"))
                btn_save_setting_as_default = gr.Button(value=i18n("save_current_settings"))
                btn_restore_default = gr.Button(value=i18n("restore_settings_default"))
